...
const incidenciasTexto = (n: number) => `${n} ${n === 1 ? "incidencia" : "incidencias"}`;
...
export function CorridaSection({
  ...
  /** Los pasos 3 y 4 del asistente (índices 2 y 3): una corrida terminada abre en el último. */
  const [paso, setPaso] = useState<2 | 3>(() =>
    recibos.length > 0 && recibos.every((r) => r.Estado === "TIMBRADO") ? 3 : 2
  );
  ...
  const editable = periodo.Estado === "BORRADOR" && timbrados.length === 0;
  const terminada = recibos.length > 0 && pendientes.length === 0 && conError.length === 0;
  const puedeRepetir = recibos.length > 0 && periodo.TipoNomina !== "E";
    ...
    <div className="space-y-4">
      ...
      <AsistentePasos
        riel={{
          ...
          folio: periodo.Nombre || `${periodo.FechaInicialPago} al ${periodo.FechaFinalPago}`,
          icono: <IconoMenu icono="corrida" />,
          pasos: rielCorrida(
            paso,
            {
              emisor: periodo.Nombre || "Sin nombre",
              periodo: `${etiquetaPeriodicidad(periodo.Periodicidad)} · paga el ${periodo.FechaPago}`,
              empleados: omitidos.length > 0 ? `${omitidos.length} fuera` : `${recibos.length} recibos`,
            },
            {
              // Los datos de la corrida se corrigen con "Editar" mientras nada
              // se haya timbrado; después ya no hay a dónde volver.
              habilitados: editable ? ["periodo", "empleados", "timbrar"] : ["empleados", "timbrar"],
              faltan: { empleados: omitidos.length },
            }
          ),
          onIr: (id) => {
            if (id === "periodo" && editable) setEditando(true);
            else if (id === "empleados") setPaso(2);
            else if (id === "timbrar" && recibos.length > 0) setPaso(3);
          },
        }}
        indice={paso}
          ...
          <Card>
            <CardBody className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Pill tone={periodo.TipoNomina === "E" ? "violet" : "neutral"}>
                  {periodo.TipoNomina === "O" ? "Ordinaria" : "Extraordinaria"}
                </Pill>
              </div>
              <p className="text-[13.5px] font-semibold text-ink">{nombreCorrida}</p>
              <p className="text-[12.5px] text-ink-3">
                Se paga el {periodo.FechaPago} · {dias(periodo.DiasPagados)} días
              </p>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: "Recibos", valor: String(recibos.length) },
                  { label: "Por timbrar", valor: String(pendientes.length) },
                  { label: "Timbrados", valor: String(timbrados.length) },
                  { label: "Con error", valor: String(conError.length), tono: conError.length > 0 },
                ].map((k) => (
                  <div key={k.label} className="rounded-xl border border-line px-3 py-2.5">
                    <span className={`block text-[20px] font-bold tabular-nums tracking-tight ${k.tono ? "text-danger" : "text-ink"}`}>
                      {k.valor}
                    </span>
              ...
              {recibos.length > 0 && (
                <div className="flex h-2 overflow-hidden rounded-full bg-line-2" aria-hidden>
                  <span className="h-full bg-ok" style={{ width: `${(timbrados.length / recibos.length) * 100}%` }} />
                  <span className="h-full bg-danger" style={{ width: `${(conError.length / recibos.length) * 100}%` }} />
                </div>
              )}
              <div className="flex items-baseline justify-between gap-3 border-t border-line pt-3">
                <span className="text-[12.5px] font-semibold text-ink">Neto total</span>
                <span className="font-mono text-lg font-bold tracking-tight text-ink">{pesos(netoTotal)}</span>
              </div>
              <div className="flex flex-wrap gap-1.5 border-t border-line pt-3">
                {editable && (
                  <>
                    <Button size="sm" variant="ghost" onClick={() => setEditando(true)} disabled={corriendo}>
                      Editar
                    </Button>
                    <Button size="sm" variant="ghost" onClick={borrar} disabled={borrando || corriendo}>
            ...
            <>
              ...
              {porRecalcular && (
                <Note tone="warn" title="Hay cambios que no se han aplicado">
                  Los recibos todavía son los de antes. Corre la nómina otra vez para que se
                  reflejen.{" "}
        ...
        pie={{
          izquierda:
            paso === 2 ? (
              <Link href="/facturas/nomina" className={buttonClass("ghost")}>
                Volver a nómina
              </Link>
            ) : (
              <Button variant="ghost" onClick={() => setPaso(2)} disabled={corriendo}>
                Atrás
              </Button>
            ),
          derecha:
            paso === 2 ? (
              <>
                ...
                <Button variant="primary" onClick={() => setPaso(3)} disabled={recibos.length === 0}>
                  Continuar
                </Button>
              </>
            ) : pendientes.length > 0 ? (
              <Button variant="primary" onClick={timbrar} disabled={corriendo || calculando || !serie}>
                {corriendo ? "Timbrando…" : `Timbrar ${pendientes.length} ${pendientes.length === 1 ? "recibo" : "recibos"}`}
              </Button>
            ) : puedeRepetir ? (
              <Button variant="primary" onClick={() => setRepitiendo(true)}>
          ...
          <div className="space-y-3">
            {recibos.length > 0 && periodo.Estado !== "CERRADO" && (
              ...
              <div className="flex justify-end">
                <Button size="sm" variant="secondary" onClick={() => setEligiendo(true)} disabled={calculando || corriendo}>
                  Agregar empleados
                </Button>
              </div>
            )}
            {recibos.length === 0 ? (
              ...
              <TablaRecibos
                ...
                quitando={quitando}
                onQuitar={quitar}
                onIncidencias={setIncidenciasDe}
              />
            )}
          </div>
        )}
              ...
              <>
                ...
                <dl className="divide-y divide-line-2 text-[13px]">
                  <div className="flex justify-between gap-3 py-2">
                    <dt className="text-ink-3">Periodo</dt>
                    <dd className="text-right font-medium text-ink">{nombreCorrida}</dd>
                  </div>
                  <div className="flex justify-between gap-3 py-2">
                    <dt className="text-ink-3">Se timbran</dt>
                    <dd className="text-right font-medium text-ink">
                      {pendientes.length} de {recibos.length} ·{" "}
                      {pesos(pendientes.reduce((a, r) => a + (parseFloat(r.Neto) || 0), 0))} de neto
                    </dd>
                  </div>
                  {timbrados.length > 0 && (
                    <div className="flex justify-between gap-3 py-2">
                      <dt className="text-ink-3">Ya timbrados</dt>
                      <dd className="text-right font-medium text-ink">{timbrados.length}, no se tocan</dd>
                    </div>
                  )}
                </dl>
                {porRecalcular && (
                  <Note tone="warn" title="Hay cambios sin aplicar">
                    Vuelve al paso anterior y recalcula antes de timbrar: los recibos todavía son los de antes.
                  </Note>
                )}
              </>
            ) : terminada ? (
              // Cuando ya no falta nada por timbrar, lo siguiente que se va a
              // querer hacer es la quincena que sigue. Vale mas ofrecerlo aqui
              // que dejar que se regrese a la lista a crearla de cero.
              <Note tone="info" title="Esta corrida ya quedó">
                Los {recibos.length} recibos están timbrados.
                {puedeRepetir &&
                  ` Cuando toque el periodo siguiente no la vuelvas a armar: repítela y te la deja lista con los mismos ${recibos.length} ${recibos.length === 1 ? "empleado" : "empleados"} y las fechas nuevas.`}
              </Note>
            ) : (
              <p className="text-[13px] text-ink-3">No hay recibos por timbrar.</p>
            )}
          </div>
        )}
      </AsistentePasos>
...
/** Los recibos de la corrida, con su desglose y sus incidencias. */
function TablaRecibos({
  ...
  onQuitar: (r: ReciboNomina) => void;
  onIncidencias: (r: ReciboNomina) => void;
}) {
  return (
    <Table>
      <thead>
            ...
            <tr className="transition hover:bg-surface-2">
              <Td>
                <button type="button" aria-label="Ver el desglose"
                  onClick={() => onAbrir(abierto === r.Id ? null : r.Id)}
              ...
              <Td className="min-w-[14rem]">
                ...
                {/* Las acciones van debajo del nombre y no en su propia columna:
                    así la tabla cabe junto al resumen de la corrida. Un recibo
                    timbrado ya no admite cambios: lo que hay que corregir es el
                    CFDI ante el SAT, no la captura. */}
                {r.Estado === "TIMBRADO" ? (
                  (incidencias[r.IdEmpleado] ?? []).length > 0 && (
                    <span className="mt-1 block text-[12px] text-ink-3">
                      {incidenciasTexto((incidencias[r.IdEmpleado] ?? []).length)}
                    </span>
                  )
                ) : (
              ...
              {/* ISR y subsidio en una sola columna: o se le retiene o se le
                  entrega, nunca las dos cosas, y así la tabla cabe en el asistente. */}
              ...
              <Td>
                {r.Estado === "TIMBRADO" ? (
                  ...
                  <Pill tone="neutral" className="whitespace-nowrap">por timbrar</Pill>
                )}
              </Td>
            </tr>
            {abierto === r.Id && (
